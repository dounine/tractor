...
class ActorStreamTest
  ...
  override protected def beforeAll(): Unit = {
    ...
    val files = Seq(
      file"/tmp/journal_${classOf[ActorStreamTest].getSimpleName}",
      file"/tmp/snapshot_${classOf[ActorStreamTest].getSimpleName}"
    )
    try {
  ...
  "updown test" should {
    ...
    "cancel timeout fail" in {
      ...
      Source
        .future(
          messageTrade.ask[MessageBehavior.Event](
            MessageBehavior.Cancel(500.milliseconds, "abc")(_)
          )(100.milliseconds)
        )
        .runWith(
          ActorSink.actorRef(
