...
import com.dounine.tractor.tools.akka.DataSource
...
class UserService(system: ActorSystem[_])
  ...
  override def valid(token: String): Option[UserModel.Session] = {
  ...
  override def info(phone: String): Future[Option[UserModel.UserInfo]] = {
    Future.successful(Option.empty[UserModel.UserInfo])
  }
